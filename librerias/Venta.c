...
void verGananciaMes(Venta venta)
{
    ...
    for(int i = 0; i <= cantidadVentas; i++)
        {
            if(venta.fecha.anio == arregloVentas[i].fecha.anio && venta.fecha.mes == arregloVentas[i].fecha.mes)
                {
                for(int f = 0; arregloVentas[i].autoAVender.letras == arregloAutos[f].patente.letras && arregloVentas[i].autoAVender.numeros == arregloAutos[f].patente.numeros; f++)
                        {
                        ganancias = ganancias + arregloVentas[i].ganancia;
                        perdidas = perdidas + arregloAutos[f].precioDeAdquisicion;
                        }
                }
        }
    ...
    if(resultado < 0)
    {
        puts("¡LAS PERDIDAS SUPERAN LAS GANANCIAS!");
    }
    puts("------------------------------------");
}
...
/*
    ...
    while(verificado == 0)
    {
        ...
        if(gananciaPositiva(venta) == 1 && gananciaMenorQueVenta(venta) == 1)
        {
            verificado = 1;
        }
